...
const i18n = {
  jp: {
    ...
    aiSub: "JRA公式コース規格に基づく最適なスタミナ目安や勝てるスキル構成をAIがアドバイスします。",
    aiInitMsg: "🤖 <strong>AIアシスタント:</strong> トレーナーさん、こんにちは！コース選びや攻略スキルについて何でもご質問ください。",
  ...
  en: {
    ...
    aiSub: "AI provides real-time strategy recommendations based on official JRA track geometry.",
    aiInitMsg: "🤖 <strong>AI Assistant:</strong> Hello Trainer! Ask me anything about optimal builds, stamina targets, or skills!",
  ...
  fr: {
    ...
    aiSub: "L'IA fournit des recommandations en temps réel basées sur la géométrie officielle des pistes de la JRA.",
    aiInitMsg: "🤖 <strong>Assistant IA:</strong> Bonjour Entraîneur ! Posez-moi vos questions sur les stratégies et compétences !",
  ...
  tw: {
    ...
    aiSub: "AI即時分析JRA賽道幾何，提供最合適的耐力目標與技能組合。",
    aiInitMsg: "🤖 <strong>AI顧問:</strong> 訓練員你好！關於賽道選擇與培育技能有任何疑問歡迎隨時提問。",
  ...
  kr: {
    ...
    aiSub: "JRA 코스 규격에 따른 최적의 스태미나 목표와 육성 스킬을 AI가 진단해 드립니다.",
    aiInitMsg: "🤖 <strong>AI 아시스턴트:</strong> 트레이너님, 안녕하세요! 육성 스킬 및 코스에 대해 궁금한 점을 물어보세요.",
...
function setupUserAiChatListeners() {
  ...
  function handleUserAiQuery() {
    ...
    setTimeout(() => {
      ...
      if (currentLang === 'fr') {
        answer = `🤖 <strong>【Assistant IA Universal Racing】</strong><br>Pour le parcours "${cName}", déclenchez les compétences d'accélération au repère de sprint (${data.spurtPos}m) !`;
      } else if (currentLang === 'en') {
        if (lower.includes("stamina") || lower.includes("heal") || lower.includes("recover")) {
          answer = `💧 <strong>【AI Training Advice】</strong><br>Currently selected course "${cName}" distance is ${data.totalDistance}m! Target stamina is minimum 850-1000+ with 1-2 Gold recovery skills!`;
        } else {
          answer = `🤖 <strong>【AI Strategy Concierge】</strong><br>Key to victory on "${cName}": Trigger max speed & acceleration skills right at the ${data.spurtPos}m spurt mark!`;
        }
      } else if (currentLang === 'tw') {
        answer = `🤖 <strong>【AI專任顧問】</strong><br>「${cName}」勝利關鍵：請於 ${data.spurtPos}m 衝刺點精準發動加速技能！`;
      } else if (currentLang === 'kr') {
        answer = `🤖 <strong>【AI 컨시어지】</strong><br>「${cName}」승리 팁: ${data.spurtPos}m 스퍼트 지점에 맞춰 최속 가속 스킬을 발동시키는 것이 핵심입니다!`;
      } else {
        if (lower.includes("スタミナ") || lower.includes("回復") || lower.includes("stamina")) {
          answer = `💧 <strong>【AI育成アドバイス】</strong><br>現在選択中の「${cName}」は距離 ${data.totalDistance}m です！スタミナ目安は最低850〜1000＋金回復スキル（円弧のマエストロ等）が1〜2つ必須となります！`;
        } else {
          answer = `🤖 <strong>【AIコンシェルジュ】</strong><br>「${cName}」での勝利のコツ：最速スパート位置（${data.spurtPos}m地点）に合わせて最高速度・最高加速スキルを発動させることがポイントです！`;
        }
      }
